package inspire.ariel.inspire.dbmanager;
...
public class RealmManager {
    ...
    private RealmResults<Treat> getTreats(Realm realm) {
        try {
            ...
            RealmResults<Treat> localTreats = realm.where(Treat.class).findAll();
    ...
    public void saveTreatsList(Collection<Treat> treatsForSaving) {
        try {
            ...
            mainThreadRealm.commitTransaction();
        } finally {
            if (mainThreadRealm != null) {
                mainThreadRealm.close();
            }
    ...
    public void removeTreat(Treat treat) {
        try {
            ...
            mainThreadRealm.executeTransactionAsync(realm -> {
                ...
                Log.d(TAG, "treat removed from db. details: " + treat.toString());
            });
        } finally {
            if (mainThreadRealm != null) {
                mainThreadRealm.close();
    ...
    private void updateRealmTreatFromOtherTreat(Treat realmTreat, Treat otherTreat) {
        ...
        realmTreat.setPurchased(otherTreat.isPurchased());
                ...
                for (final Treat localTreat : localTreats) {
                    ...
                    if (match != null) {
                        ...
                        if (!match.equals(localTreat)) {
                            Log.d(AppStrings.REALM_SYNC_SERVER_TAG, "Scheduling update for treat: " + match.toString());
                            updateRealmTreatFromOtherTreat(localTreat, match);
                        } else {
                            Log.d(AppStrings.REALM_SYNC_SERVER_TAG, "Treat equality in databases. no update required for " + match.toString());
                        }
                    } else {
                        // Entry doesn't exist. Remove it from the database.
                        RealmResults<Treat> result = realm1.where(Treat.class).equalTo(AppStrings.KEY_OBJECT_ID, localTreat.getObjectId()).findAll();
                        result.deleteFirstFromRealm();
